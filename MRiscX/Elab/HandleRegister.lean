...
def getCorrespondingRegister (r : (TSyntax `mriscx_registers)) : TermElabM Expr :=
  match r with
  | `(mriscx_registers | x0) =>
    return mkRegisterName (RegisterName.mk 0 "x0")
  | `(mriscx_registers | zero) =>
    return mkRegisterName (RegisterName.mk 0 "zero")
  | `(mriscx_registers | x1) =>
    return mkRegisterName (RegisterName.mk 1 "x1")
  | `(mriscx_registers | ra) =>
    return mkRegisterName (RegisterName.mk 1 "ra")
  | `(mriscx_registers | x2) =>
    return mkRegisterName (RegisterName.mk 2 "x2")
  | `(mriscx_registers | sp) =>
    return mkRegisterName (RegisterName.mk 2 "sp")
  | `(mriscx_registers | x3)
  | `(mriscx_registers | gp)
  | `(mriscx_registers | x4)
  | `(mriscx_registers | tp)
