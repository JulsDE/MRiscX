...
--   match t with
--     | `(mriscx_Instr | la $r:mriscx_registers, $addr:num_or_ident
--     )
--     | `(mriscx_Instr | la $r:mriscx_registers, $addr:num_or_ident;) =>
...
--     | `(mriscx_Instr | li $r:mriscx_registers, $v:num_or_ident
--     )
--     | `(mriscx_Instr | li $r:mriscx_registers, $v:num_or_ident;) =>
--       let mut exprOfReg ← getCorrespondingRegister r
...
--     | `(mriscx_Instr | lw $dst:mriscx_registers, $addr:num_or_ident
--     )
--     | `(mriscx_Instr | lw $dst:mriscx_registers, $addr:num_or_ident;) =>
--       let exprOfDst ← getCorrespondingRegister dst
--       let exprOfAddr ← parseMriscxNumOrIdent addr
--       return (mkAppN (.const `Instr.LoadWordImmediate []) #[exprOfDst, exprOfAddr])
--     | `(mriscx_Instr | lw $dst:mriscx_registers, $off:num($addr:mriscx_registers)
--     )
--     | `(mriscx_Instr | lw $dst:mriscx_registers, $off:num($addr:mriscx_registers);) =>
--       let exprOfDst ← getCorrespondingRegister dst
--       let exprOfAddr ← getCorrespondingRegister addr
--       return (mkAppN (.const `Instr.LoadWordReg []) #[exprOfDst, exprOfAddr])
--     | `(mriscx_Instr | sw $reg:mriscx_registers, $off:num($dst:mriscx_registers)
--     )
--     | `(mriscx_Instr | sw $reg:mriscx_registers, $off:num($dst:mriscx_registers);) =>
--       let exprOfReg ← getCorrespondingRegister reg
--       let exprOfDst ← getCorrespondingRegister dst
...
partial def getLabelMapFromSyntax (syn : TSyntax `mriscx_syntax): TermElabM LabelMap := do
  ...
  | `(mriscx_syntax | mriscx
    ...
    let mut labelMap : LabelMap := PMap.empty
