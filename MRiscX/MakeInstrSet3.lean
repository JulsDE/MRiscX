import MRiscX.MakeInstrSet_Import
...
instance : ToString CtorSpec where
  toString c :=
    s!"CtorSpec(ref := {reprStr c.ref}, ctorName := {reprStr c.ctorName}, pieces := {reprStr c.pieces}, sem := {reprStr c.sem})"
...
private def mkExecuteAlt
  ...
  let sem := spec.sem
  let rhs : TSyntax `term ← `(term| ($sem:term) $stateIdent:ident)
...
private def mkExecuteCmd
  ...
  `(command|
    def $execName:ident ($stateIdent : MState) ($instrIdent : $typeName:ident) : MState :=
...
make_InstrSet Instr execute
  ...
  Jump:
    { syntax : j (lbl:label),
      semantics: fun ms => ms.jump lbl }
  -- Panic:
  --   { syntax : panic,
  --     semantics: fun ms => ms.setTerminated true }
